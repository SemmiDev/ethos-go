...
export const habitsAPI = {
  ...
  startVacation: async (habitId, reason) => {
    const response = await apiClient.post('/habit-vacations', { habit_id: habitId, reason, start_date: new Date().toISOString() });
    return response.data;
  },

  // End Vacation Mode
  endVacation: async (vacationId) => {
    // Usually we update the vacation record with an end_date. Or calling a specific endpoint.
    // Based on backend implementation: PUT /habit-vacations/{id}/end
